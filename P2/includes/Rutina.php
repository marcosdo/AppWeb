...
class Rutina {
    ...
    public function comprobarRutina() { 
        ...
        if($resultado){
            if($resultado->num_rows > 0){
                ...
                if($fila['objetivo'] != $this->_objetivo || $fila['nivel'] != $this->nivel || $fila['dias'] != $this->dias)
                    this->crearRutina(true);
            }
            else $this->crearRutina(false); 
    ...
    private function crearRutina($update){
        ...
        for ($i = 1; $i < $this->_dias+1; $i++) {
            ...
            if ($i == 4) $cont = 1;
            if ($i >= 1 && $i <= 3) {
               $this->fill_array($cont, 2, $arrayaux, $stringaux);
            }
            else {
                $this->fill_array($cont, 3, $arrayaux, $stringaux);
            }
        ...
        $BD = Aplicacion::getInstance()->getConexionBd();
        if(!$update) 
            $query = "INSERT INTO planificacion (id_usuario, rutina, eobjetivo, dias, nivel) VALUES ('$this->_id', '$guardar', '$this->_objetivo', '$this->_dia', '$this->_nivel')";
        else
            $query = "UPDATE planificacion SET planificacion.rutina = '$guardar', planificacion.nivel = '$this->_nivel', planificacion.dias = '$this->_dia',  planificacion.eobjetivo = '$this->_objetivo'";
    ...
    private function fill_array(&$cont, $nveces , &$arrayaux, &$stringaux) {
        ...
        for ($i = 0; $i < $nveces; $i++){
            ...
            while ($fila = $resultado->fetch_assoc()){
                if($j < $this->_ejerciciosdia){
                    ...
                    if($j+1 == $this->_ejerciciosdia && $i+1 == $nveces) $stringaux .= $fila['nombre'];
                    else {
